...
async def entrypoint(ctx: JobContext):
    ...
    try:
        ...
        # If there's an error, try to end the call gracefully
        try:
            if 'session' in locals():
                await session.say("I'm sorry, but I'm experiencing technical difficulties. Please call back later.", allow_interruptions=False)
        except Exception as session_error:
            logger.error(f"Error sending error message: {session_error}")
        
        # Re-raise the exception to let LiveKit handle it
        raise
